...
export class ChatMessage {
  ...
  readonly isStreamingTool = computed(
    () => this.role() === 'tool' && this.message().status() === MessageStatus.STREAMING
  );

  readonly isToolComplete = computed(
    () => this.role() === 'tool' && this.message().status() === MessageStatus.COMPLETE
  );

  readonly isLoading = computed(() => this.isEmptyAssistant() || this.isStreamingTool());
}
