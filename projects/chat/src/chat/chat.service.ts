...
export class ChatService {
  ...
  readonly isLastMessageUserAndSending = computed(() => {
    ...
    return (
      ...
      [MessageStatus.SENDING, MessageStatus.SENT].includes(messages[messages.length - 1].status())
    );
  });
  ...
  readonly isLoading: Signal<boolean> = computed(() => {
    return this.isLastMessageUserAndSending() || this.isSomeMessageAIAndStreamingButEmpty();
  });
      ...
      next: (event: BaseEvent) => {
        switch (event.type) {
          ...
          case EventType.TEXT_MESSAGE_CHUNK: {
            ...
            if (!this.assistantMessage || this.assistantMessage.messageId !== ev.messageId) {
              this.assistantMessage = new AssistantMessage(ev);
              this._messages.update((msgs) => [...msgs, this.assistantMessage!]);
            } else this.assistantMessage.content.update((content) => content + ev.delta);
            break;
          }

          case EventType.TOOL_CALL_CHUNK: {
            const ev = event as ToolCallChunkEvent;
            if (!this.toolCallMessage || this.toolCallMessage?.toolCallId !== ev.toolCallId) {
            ...
            } else this.toolCallMessage.content.update((content) => content + ev.delta);
            break;
          }
