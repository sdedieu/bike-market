...
export class ToolService {
  ...
  readonly completedCallMessages = computed(() =>
    this.toolCallMessages().filter((msg) => msg.status() === MessageStatus.COMPLETE)
  );
  ...
  readonly routerNavigateToolMessages = computed(() =>
    this.completedCallMessages().filter((msg) => msg.toolCallName === ToolCallName.ROUTER_NAVIGATE)
  );
  ...
  readonly currentRoute = computed(() => {
    ...
    const { route } = routerNavigateToolMessages[routerNavigateToolMessages.length - 1].result();
    return route;
  });
